...
class payment_page:
    def __init__(self, top=None, bookid=None):
        ...
        self.Label_user.configure(
            font="-family {TH Niramit AS} -size 18 -weight bold")
        self.Label_user.configure(foreground="#ffffff")
        waiteruserid = backend.Book_Flight.getBookDetailById(waitbooktid)[
            'User_ID']
        self.Label_user.configure(
            text=backend.User.getUserById(waiteruserid)['username'])
        ...
        waiteruserid = backend.Book_Flight.getBookDetailById(waitbooktid)[
            'User_ID']
        self.Label_ID.configure(
            text=backend.User.getUserById(waiteruserid)['id_card'])

        self.Label_departure = tk.Label(self.top)
        self.Label_departure.place(
            relx=0.481, rely=0.593, height=41, width=248)
        ...
        self.Label_departure.configure(
            font="-family {TH Niramit AS} -size 18 -weight bold")
        self.Label_departure.configure(foreground="#ffffff")
        self.Label_departure.configure(text=backend.Book_Flight.getBookDetailById(waitbooktid)[
                                       'Source'] + " > " + backend.Book_Flight.getBookDetailById(waitbooktid)['Destination'])
        ...
        Flight_ID = backend.Book_Flight.getBookDetailById(waitbooktid)[
            'Flight_ID']
        self.Label_price.configure(
            text=backend.Flight.getFlightById(Flight_ID)['price'])
        ...
        self.Button_comfirm.configure(command=self.confirm)
    ...
    def confirm(self):
        backend.Book_Flight(self.Book_id).pay()
        main.changePage(self.top, history.History_page)

    def cancel(self):
        backend.Book_Flight(self.Book_id).cancel()
        main.changePage(self.top, history.History_page)
